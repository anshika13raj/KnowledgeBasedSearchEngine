...
const ChatInterface = ({ pdfName, messages, isIndexing, isLoading }) => {
  const messagesEndRef = useRef(null);
  ...
  return (
    <div className="flex-1 overflow-y-auto glass-card p-4 space-y-4">
      {isIndexing && <div>Processing your PDF...</div>}
      {messages.map((msg, i) => (
        <div key={i} className={`flex ${msg.type === 'user' ? 'justify-end' : 'justify-start'}`}>
          <div className={`p-3 rounded-xl max-w-[80%] ${
            msg.type === 'user' ? 'bg-blue-500/20' : msg.type === 'bot' ? 'bg-white/10' : 'bg-gray-400/20'
          }`}>
            {msg.content}
          </div>
        </div>
