...
// Health check
app.get('/api/health', (req, res) => {
  res.json({ status: 'OK', message: 'RAG Backend is running', port: PORT });
});

// Test endpoint
app.get('/', (req, res) => {
  res.json({ message: 'RAG Backend is live!', endpoints: ['/api/health', '/api/upload', '/api/query'] });
});
...
app.post('/api/upload', upload.single('pdf'), async (req, res) => {
  ...
  try {
    ...
    sessions.set(sessionId, { filename: req.file.originalname, uploadTime: new Date().toISOString() });
    console.log(`Indexing completed for session ${sessionId}`);

    res.json({ success: true, message: 'PDF uploaded and indexed successfully', sessionId, filename: req.file.originalname });
  } catch (error) {
    console.error('Upload/Index error:', error);
...
app.post('/api/query', async (req, res) => {
  ...
  if (!sessions.has(sessionId)) return res.status(404).json({ error: 'Session not found. Upload a PDF first.' });

  try {
    console.log(`Processing query for session ${sessionId}: ${question}`);
    const response = await queryRAG(question, sessionId);
    res.json({ success: true, answer: response, sessionId });
  } catch (error) {
    console.error('Query error:', error);
