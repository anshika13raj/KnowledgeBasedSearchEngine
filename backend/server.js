...
app.use(cors({
  origin: '*', // You can replace * with your frontend URL if you want stricter security
  credentials: true
}));
...
// ✅ Simple health route (for Render checks)
app.get('/health', (req, res) => {
  res.json({
    status: 'OK',
    message: 'RAG Backend is healthy',
    timestamp: new Date().toISOString(),
    port: PORT
  });
});
...
app.get('/', (req, res) => {
  res.json({
    message: 'RAG Backend is live!',
    endpoints: ['/health', '/api/health', '/api/upload', '/api/query'],
    timestamp: new Date().toISOString()
  });
});
